/*
 * DIAGNOSTIC-ONLY tool. Real-candidate-generation variant of
 * ground_truth_calcsimscore_batch_spa.c.
 *
 * Replaces this project's own approximated candidate-matching rule
 * (Hamming<=60 && ratio<=0.85*second) with the REAL rule found via
 * disassembly + live gdb register verification of FtVerifyTwoTemplate's
 * inline candidate-generation loop (FtAlg.c, ~0xc3d61-0xc3f6e) -- see
 * research/PROTOCOL.md "Alignment/correspondence root-cause investigation":
 *   marginTable = [64, 64, 64, 12, 8, 6, 3, 2, 1]
 *   bucket = floor(secondBestDistance / 32), clamped to 8
 *   ACCEPT iff secondBestDistance - bestDistance > marginTable[bucket]
 * This retains the already-confirmed-faithful distance-consistency-graph
 * alignment stage (|distA-distB|<2.0, max-degree seed) unchanged.
 *
 * Also retains the SPA-smoothing fix from the prior variant:
...
/* REAL, confirmed acceptance rule (see research/PROTOCOL.md "Alignment/
 * correspondence root-cause investigation" -- disassembly + live gdb
 * register verification of FtVerifyTwoTemplate's inline candidate loop):
 * bucket on the SECOND-best distance, accept iff second-best exceeds best
 * by more than marginTable[bucket]. Replaces this project's own prior
 * ratio<=0.85 approximation. */
static const int kRealMarginTable[9] = {64, 64, 64, 12, 8, 6, 3, 2, 1};
static int real_candidate_accept(int best, int second)
{
    int bucket = second / 32;
    if (bucket > 8) bucket = 8;
    return (second - best) > kRealMarginTable[bucket];
}
                       ...
                       float *outScore, int *outCand, int *outN)
{
    ...
    for (i = 0; i < na; i++) {
        ...
        if (real_candidate_accept(best, second)) { cand[ncand].ia = i; cand[ncand].ib = bestj; ncand++; }
    }
    *outCand = ncand;
