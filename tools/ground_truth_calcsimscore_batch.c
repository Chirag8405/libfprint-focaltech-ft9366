/*
 * DIAGNOSTIC-ONLY tool. Batch version of ground_truth_calcsimscore.c:
 * loads N real captures ONCE (real FtSegmentByLocalVariance masks, real
 * FtGenBinImg binarization, real FtGetMfbFeatures keypoints), then computes
 * the REAL FtCalcSimScore for every pairwise combination, using this
 * project's own validated alignment estimator. Built to test the varied_set
 * dataset (research/captures/varied_set/) across a full distribution of
 * same-finger and different-finger pairs, not just a handful of examples --
 * see research/PROTOCOL.md.
 *
 * NEVER shipped, NEVER used at runtime -- diagnostic-only.
 *
 * Usage: ground_truth_calcsimscore_batch <so_path> label1:file1.raw label2:file2.raw ...
 *   Files sharing the same label are treated as the SAME physical finger.
 *
 * Build: gcc -O0 -g -Wall ground_truth_calcsimscore_batch.c -o ground_truth_calcsimscore_batch -ldl -lm
 */
#define _GNU_SOURCE
#include <stdio.h>
#include <stdlib.h>
